...
snit::type IconImage {
    ...
    typeconstructor {
        ...
        set unknownimg [image create photo -file [file join $icondir unknown.gif]]
        snit::enum ftypes -values {any ppm gif xbm}
    }
    typemethod insert {_self args} {
        ...
        switch $filetype {
            ...
            ppm {
                set xbmfile {}
                set giffile {}
                set ppmfile [file join $_icondir $_name.ppm]
            }
            any {
                set xbmfile [file join $_icondir $_name.xbm]
                set giffile [file join $_icondir $_name.gif]
        ...
        #puts stderr "*** $type insert $_self: ppmfile file is $ppmfile"
        #puts stderr "*** $type insert $_self: giffile file is $giffile"
        #puts stderr "*** $type insert $_self: xbmfile file is $xbmfile"
        if {$ppmfile ne {} && [file exists $ppmfile]} {
            set imagemap($_name) [image create photo -file $ppmfile]
